...
| s18 | GitOps | ArgoCD, declarative deploys, drift detection + auto-revert (`selfHeal`), App of Apps pattern, sync waves + hooks (PreSync migration Job from s15), `ApplicationSet`; contrast push (CI/CD) vs pull (GitOps); closes the loop git → ArgoCD → Argo Rollouts → Istio |
